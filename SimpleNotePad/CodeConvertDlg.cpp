...
void CCodeConvertDlg::InitComboBox(CComboBox& combo_box)
{
    ...
    for (const auto& item : CONST_VAL->CodeList())
    {
        combo_box.AddString(item.name);
    }
    if (!CONST_VAL->CodeList().empty())
...
void CCodeConvertDlg::OnBnClickedButton1()
{
    ...
    if (index < 0 || index >= static_cast<int>(CONST_VAL->CodeList().size()))
        return;
    CodeType code_type_input{ CONST_VAL->CodeList()[index].code_type };
    UINT code_page_input{ CONST_VAL->CodeList()[index].code_page };
    ...
    if (index < 0 || index >= static_cast<int>(CONST_VAL->CodeList().size()))
        return;
    CodeType code_type_output{ CONST_VAL->CodeList()[index].code_type };
    UINT code_page_output{ CONST_VAL->CodeList()[index].code_page };
