...
{
    m_view->ClearAllMark(ScintillaEditor::MarkStyle::MATCHED_BRACKETS);
        ...
        if (left_bracket_pos >= 0)
        {
            ...
            for (int i = left_bracket_pos + 1; i < length; i++)
            {
                ...
                if (ch == item.second)
                {
                    ...
                    {
                        //找到右括号，将括号对标记
                        m_view->SetMark(ScintillaEditor::MarkStyle::MATCHED_BRACKETS, left_bracket_pos, 1);
                        m_view->SetMark(ScintillaEditor::MarkStyle::MATCHED_BRACKETS, i, 1);
                        break;
                    }
                    left_bracket_count--;
                }
        ...
        if (right_bracket_pos >= 0)
        {
            ...
            for (int i = right_bracket_pos - 1; i >= 0; i--)
            {
                ...
                if (ch == item.first)
                {
                    ...
                    {
                        //找到左括号，将括号对标记
                        m_view->SetMark(ScintillaEditor::MarkStyle::MATCHED_BRACKETS, right_bracket_pos, 1);
                        m_view->SetMark(ScintillaEditor::MarkStyle::MATCHED_BRACKETS, i, 1);
                        break;
                    }
                    right_bracket_count--;
...
void CEditorHelper::BracketsAutoComp(char left_bracket)
{
    ...
    if (pos == end)
    {
        for (const auto& item : matched_characters)
        {
            ...
            {
                m_view->Paste(QString(item.second));
...
{
    ScintillaEditor::UndoRedoActionLocker locker(m_view);

    //添加/删除单行注释（使用单行注释的条件是当前语言有单行注释，并且编辑器没有选中内容）
    if (!comment.line.isEmpty() && m_view->IsSelectionEmpty())
    {
        int start{}, end{};
        m_view->GetCurLinePos(start, end);
        //查找单行注释
    ...
    //如果选中的是整行，则对每行使用单行注释
    else if (!comment.line.isEmpty() && m_view->IsFullLineSelected())
    {
...
{
    //m_view->ClearAllMark(ScintillaEditor::MarkStyle::HTML_MARKS);
...
void CEditorHelper::AutoShowCompList(const CLanguage& languange)
{
    ...
    int startpos = m_view->send(SCI_WORDSTARTPOSITION, pos - 1);//当前单词起始位置
    int endpos = m_view->send(SCI_WORDENDPOSITION, pos - 1);//当前单词终止位置
    QString word = m_view->GetText(startpos, endpos);
    QString matched_list = GetMatchedCompList(languange, word);
    if (!matched_list.isEmpty() && !word.isEmpty())
    {
        m_view->send(SCI_AUTOCSHOW, word.size(), sptr_t(matched_list.toUtf8().constData()));
    }
}

QString CEditorHelper::GetMatchedCompList(const CLanguage& languange, const QString &str)
{
    //拆分所有关键字
    QSet<QString> keywords_list;
    for (const auto& keywords : languange.m_keywords_list)
    {
        QStringList keyWordsSplit = keywords.split(' ');
        for (const auto& str : keyWordsSplit)
