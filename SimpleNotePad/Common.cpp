...
void CCommon::SendProcessMessage(HWND hwnd, ProcessMsgType msg_id, const std::string& msg_data)
{
    //通过WM_COPYDATA消息向已有进程传递消息
    COPYDATASTRUCT copy_data;
    copy_data.dwData = static_cast<UINT>(msg_id);
    copy_data.cbData = msg_data.size();
    copy_data.lpData = (const PVOID)msg_data.c_str();
    ::SendMessage(hwnd, WM_COPYDATA, 0, (LPARAM)&copy_data);
}
...
void CCommon::ParseProcessMessage(COPYDATASTRUCT* copy_data, ProcessMsgType& msg_id, std::string& msg_data)
{
    if (copy_data != nullptr)
    {
        msg_id = static_cast<ProcessMsgType>(copy_data->dwData);
        int length = copy_data->cbData;
