...
{
	bool tagFound = false;
    int caret = static_cast<int32_t>(_pEditView->send(SCI_GETCURRENTPOS));
	...
	do
	{
        ...
        styleAt = _pEditView->send(SCI_GETSTYLEAT, (uptr_t)openFound.start);
	...
	if (openFound.success && styleAt != SCE_H_CDATA)
	{
		...
		if (!closeFound.success)
		{
            ...
            int nextChar = static_cast<int32_t>(_pEditView->send(SCI_GETCHARAT, openFound.start + 1));
			...
			if ('/' == nextChar)
			{
                ...
                int docLength = static_cast<int32_t>(_pEditView->send(SCI_GETLENGTH));
				...
				std::string tagName;
                nextChar = static_cast<int32_t>(_pEditView->send(SCI_GETCHARAT, position));
...
XmlMatchedTagsHighlighter::FindResult XmlMatchedTagsHighlighter::findOpenTag(const std::string& tagName, int start, int end)
{
	...
	do
	{
		...
		if (result.success)
		{
            nextChar = static_cast<int32_t>(_pEditView->send(SCI_GETCHARAT, result.end));
            styleAt = static_cast<int32_t>(_pEditView->send(SCI_GETSTYLEAT, result.start));
		...
		if (result.success)
		{
			...
			if (styleAt != SCE_H_CDATA && styleAt != SCE_H_SINGLESTRING && styleAt != SCE_H_DOUBLESTRING && styleAt != SCE_H_COMMENT) // If what we found was in CDATA section, it's not a valid tag.
			{
				...
				else if (isWhitespace(nextChar))  // Otherwise, if it's whitespace, then allow whitespace until a '>' - any other character is invalid.
				{
					...
					{
						++whitespacePoint;
                        nextChar = static_cast<int32_t>(_pEditView->send(SCI_GETCHARAT, whitespacePoint));
...
XmlMatchedTagsHighlighter::FindResult XmlMatchedTagsHighlighter::findText(const char *text, int start, int end, int flags)
{
    ...
    int result = static_cast<sptr_t>(_pEditView->send(SCI_FINDTEXT, (uptr_t)flags, (sptr_t)(&search)));
	if (-1 == result)
	{
...
void XmlMatchedTagsHighlighter::tagMatch(bool doHiliteAttr) 
{
    ...
    _pEditView->ClearAllMark(ScintillaEditor::MarkStyle::HTML_MARKS);
    ...
    //	const int caret = 1 + static_cast<int32_t>(_pEditView->send(SCI_GETCURRENTPOS)); // +1 to deal with the case when the caret is between the angle and the question mark in "<?" (or between '<' and '%').
	...
	if (getXmlMatchedTagsPos(xmlTags))
	{
		...
		{
            _pEditView->SetMark(ScintillaEditor::MarkStyle::HTML_MARKS,  xmlTags.tagCloseStart, xmlTags.tagCloseEnd - xmlTags.tagCloseStart);
			// tag close is present, so it's not single tag
			openTagTailLen = 1;
		}

		// Colourising the open tag
        _pEditView->SetMark(ScintillaEditor::MarkStyle::HTML_MARKS,  xmlTags.tagOpenStart, xmlTags.tagNameEnd - xmlTags.tagOpenStart);
        _pEditView->SetMark(ScintillaEditor::MarkStyle::HTML_MARKS,  xmlTags.tagOpenEnd - openTagTailLen, openTagTailLen);
        ...
        if (doHiliteAttr)
		{
			...
			//{
            //	_pEditView->send(SCI_INDICATORFILLRANGE,  attributes[i].first, attributes[i].second - attributes[i].first);
			//}
        }
		...
		//{
        //	int columnAtCaret  = int(_pEditView->send(SCI_GETCOLUMN, xmlTags.tagOpenStart));
        //	int columnOpposite = int(_pEditView->send(SCI_GETCOLUMN, xmlTags.tagCloseStart));

        //	int lineAtCaret  = int(_pEditView->send(SCI_LINEFROMPOSITION, xmlTags.tagOpenStart));
        //	int lineOpposite = int(_pEditView->send(SCI_LINEFROMPOSITION, xmlTags.tagCloseStart));
        ...
        //		_pEditView->send(SCI_SETHIGHLIGHTGUIDE, (columnAtCaret < columnOpposite)?columnAtCaret:columnOpposite);
		//	}
		//}
	}
